            ...
            foreach ($lb in $lbPolicies) {
                try {
                    ...
                    $out.AluaDevices += [pscustomobject]@{
                        ...
                        LoadBalancePolicy = [int]$policy.LoadBalancePolicy
        ...
        if ($diskPolicies.Count -gt 0) {
            ...
            foreach ($dp in $diskPolicies) {
                ...
                Add-Result (New-Finding -HostName $hostName -Category "Pure Runtime Connectivity" `
                    ...
                    -Verification "Re-run the audit after any approved policy change and confirm the per-device policy matches the Pure recommendation for the observed path count.")
            }
        }
        else {
            Add-Result (New-Finding -HostName $hostName -Category "Pure Runtime Connectivity" `
...
'@
    ...
    [void]$mpioSummary.AppendLine('<div class="small" style="margin-top:8px;">The summary shows host-level MPIO readiness and the maximum observed path count for any correlated Pure device on each host. Windows supports a maximum of 32 MPIO paths per device. Once Pure volumes are presented, per-device path-limit, policy, and ALUA validation remain in Detailed Results.</div>')
